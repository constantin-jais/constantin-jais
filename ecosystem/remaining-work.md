...
> **⚠ SUPERSEDED BY THE BIG BANG (2026-07-16) — reconciled 2026-07-17 per control-plane ADR 0047.**
> The libre-ai org was frozen into the `libre-ai/libre-ai` monorepo; the target stack is now rebuilt there under 26 locked work-packages (`docs/transformation/G1-WORK-PACKAGES.md`). Everything below this banner is **frozen as the pre-Big-Bang final state** — do not execute items from this file. The reconciliation table maps each area to its disposition; live meta triggers stay in `plans/cold-backlog.md`.

## Big Bang reconciliation (2026-07-17)

| Area of this file                         | Disposition              | Where it lives now                                                                                                                                                                              |
| ----------------------------------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Wave table — lm plans (1a, 2a)            | absorbed                 | Sessions app: `WP-G3-S01` (runtime work already fed the G1 Sessions spec)                                                                                                                       |
| Wave table — governance wave (1b)         | shrunk, meta-survivor    | governed repos are archived; governance scope collapses to this control-plane repo (monorepo protects itself: rulesets, secret scanning)                                                        |
| Wave table — gear-loader hardening (1b)   | superseded               | context-kit is archive-only; ingestion concerns re-enter via the Knowledge Engine (`WP-G2-K01`) and future approved packages                                                                    |
| Wave table — portal-core hardening (1b)   | absorbed                 | `packages/ui` + Bun template (`WP-G2-W01`), per `REPOSITORY-MAP` (client-kit → `packages/*`)                                                                                                    |
| Wave table — cos rebuild I13/I14 (2b)     | caduc                    | Website was rebuilt, qualified and archived during G0/G1; hosting happens once at `WP-G4-I01`/`WP-G4-C01` (single cutover — no per-product Clever Cloud)                                        |
| Wave table — canvas MVP (2c)              | absorbed                 | Specifications app: `WP-G3-F01`                                                                                                                                                                 |
| Wave table — feed-mind waivers (2d)       | absorbed                 | Radar: `WP-G3-R01`; the 2026-09-30 RustSec deadline dies with the archived repo — verify equivalents in the monorepo supply-chain gates                                                         |
| Wave table — ai-practices convergence (3) | absorbed                 | Practices app: `WP-G3-P01`                                                                                                                                                                      |
| Wave table — cos-matic Biscuit (3)        | absorbed                 | Biscuit capability: `WP-G2-Z01`; orchestration itself waits for the separate composite Specification Lock                                                                                       |
| Wave table — cable↔depot E2E (4)          | absorbed (was delivered) | Proof and Artifact: `WP-G2-P01` (`crates/artifact`, `crates/proof`)                                                                                                                             |
| Wave table — harness fixtures (4)         | deferred                 | future orchestrator/harness package behind its own Specification Lock; inputs manifest: `plans/orchestrator-lock-inputs.md`                                                                     |
| Wave table — wrench-db-inspect gates (5)  | absorbed                 | `WP-G2-D01` (PostgreSQL/RLS/retention/deletion platform) + `crates/proof`/`verification/`                                                                                                       |
| P0 control plane & naming                 | meta-survivor (reduced)  | this repo's own hygiene; layer names survive as roles (ADR 0047)                                                                                                                                |
| P0a public topology                       | absorbed                 | monorepo ADR series + Website publication gates (control-plane ADR 0046 remains the domain-topology origin)                                                                                     |
| P0b stack validation matrix               | superseded               | monorepo toolchain/workspace decisions (`WP-G2-T01`) and its own quality gates                                                                                                                  |
| P1 Portal proof                           | absorbed                 | `packages/ui`, `WP-G2-W01`, native adapters post-cutover                                                                                                                                        |
| P2 Gear substrate                         | superseded/deferred      | `WP-G2-K01` (knowledge engine); context/memory crates require newly approved packages; conversational memory design ready in `specs/shared/agent-conversational-memory-design.md` (Accepted R2) |
| P3 Wrench evidence                        | absorbed                 | `crates/proof`, `verification/` (`WP-G2-P01`, conformance packages `WP-G3-X01..X03`)                                                                                                            |
| P4 Bolt integration                       | deferred                 | composite orchestrator Specification Lock (see `plans/orchestrator-lock-inputs.md`)                                                                                                             |
| P5 first product slice                    | superseded               | G3 rebuilds all nine apps in parallel (`g3-1`), no first-slice sequencing                                                                                                                       |
| P6 native paths                           | deferred                 | post-cutover (G5 distribution; native shells when proven)                                                                                                                                       |
