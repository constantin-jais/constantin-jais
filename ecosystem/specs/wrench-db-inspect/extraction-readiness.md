...
Extraction must preserve the contract-first posture: specs, fixtures, reports, and gates remain authoritative; code follows them.
...
- [ ] Bolt/CI consumes `data.summary.gate_blocked`, `data.findings[*].gate`, `data.report_gate`, and `meta.redaction`.
- [ ] Bolt/CI does not re-parse raw SQL.
...
5. Add license file and dependency license audit.
6. Tag initial version `v0.1.0-prototype` or equivalent.
7. Update ecosystem docs to point to the standalone repo.
8. Keep specs in ecosystem as control-plane contracts until the repo owns its own mirrored docs.
