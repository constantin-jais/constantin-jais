...
This document defines how Bolt and CI pipelines should run `wrench-db-inspect`, consume its reports, and decide gates without re-parsing SQL or duplicating DB-security policy inside each Rumble.

`wrench-db-inspect` produces evidence. Bolt/CI selects the gate profile and records the result.
...
No secrets, DSNs, row data, raw embeddings, prompts, or PII may be passed as report metadata.
...
Bolt/CI must read only the report contract, not raw SQL:
...
CI must not upload raw DB dumps containing row data or credentials.

## Bolt Gate Record Shape

Bolt should record a gate result similar to:
...
1. Run in `local`/warning mode on each Rumble to collect false positives.
2. Require manifests for all Postgres-backed Rumbles.
3. Enable `pull_request` for P0 critical/high.
4. Enable `protected_branch` once unknown states are low.
5. Enable `release` with strict waiver and redaction behavior.
