...
| `rumble-cos` | `BLOCKED` | `specs/rumble-cos/00-product-charter.md` only | Specs are effectively not started: missing roles, journeys, screens/actions, domain/data model, APIs, events, security/RGPD, acceptance tests, and handoff/export path. | Complete the product spec template before any harness package. |
...
| `rumble-cos` | What is the product unit and MVP slice? | Product | Yes. |
