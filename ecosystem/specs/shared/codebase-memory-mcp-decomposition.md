...
All verdicts below were proposed and then **accepted on 2026-07-02**; the implementation gate is lifted. The first increment (`gear-memory` P1) may proceed.
...
| E11 | `detect_changes` (git diff → affected symbols + risk class) + `FILE_CHANGES_WITH` co-change mining                                                                                                               | Impact analysis for changes                      | bolt           | rebuild (later)                         | Resolved 2026-07-02 (split): the mechanical "affected symbols for a diff" lookup is a `gear-memory` query; risk classification and gating live in cos-matic; `wrench-inspect` only if a dedicated evidence report emerges. Dedicated increment after P1. |
...
| E17 | Team-shared artifact: `.codebase-memory/graph.db.zst` committed + `.gitattributes` merge policy                                                                                                                  | Clone-and-go warm index                          | gear           | knowledge                               | Resolved 2026-07-02: bundle export/import are `gear-memory` CLI capabilities; artifact custody (checksums, retention, revocation, distribution) is `gear-depot`; no committed binary snapshots in repos (derived, regenerable). No increment now.        |
...
- **Progressive indexing ladder** (decision 2026-06-30, Proposed: catalog → full-text → graph → tree-sitter `CodeMap` → optional vector): amended 2026-07-02 — rungs between catalog and vector are commutative within a single SQLite engine; P1 lands on the graph rung, FTS5 joins the same file later; only the vector rung stays strictly last.
...
## First increment (validated 2026-07-02)

`gear-memory` P1 = E4 + E6 + E7 (+ E1 flush pattern). Stage 0 is merged (`gear-memory#1`), so P1 branches from `main`. The storage engine is selected by a bounded micro-benchmark (rusqlite `bundled` vs stoolap vs redb; fixed ingest+query workload) with inspectability as the lexicographic first criterion — honoring the standing stoolap audit note "benchmark before any storage decision". Verdict, database-location policy (git-ignored `./.gear-memory/`, explicit `--db`, no hidden global cache), the dev-only fixture-generator guardrail, and the `codebase-memory-mcp` = design-reference positioning go to `gear-memory` `docs/adr/0002-…`, which also supersedes the stale upstream-tracking section of ADR-0001 (`basemind` demoted to knowledge reference).
...
1. Ladder order → rungs between catalog and vector are **commutative within one SQLite engine**; only the vector rung stays strictly last. The 2026-06-30 ladder decision is amended accordingly.
