...
- Feed Radar removed `RUSTSEC-2026-0173` by updating `validator_derive` to 0.20.1. Its first non-fixture traversal now imports two OPML sources, revalidates an exact HTTPS host allowlist across redirects, streams under byte/item limits, persists only replay hashes, produces a client-safe export and replays the static Dioxus review on Chromium/Firefox/WebKit. The dated proof remains local with `publication_authorized: false`; interactive import, hosted network sandbox, canonical URL and reviewed public evidence are still absent, so Radar remains `discovery`.
- A three-commit Biscuit patch is locally upstream-ready, removes `proc-macro-error2`, and passes Biscuit, Agent Factory and Sessions tests, but is unpublished. Agent Factory and Sessions retain the build-time waiver until an official release is adopted. Both removed `RUSTSEC-2023-0071` from their locked graphs by selecting the local AWS-LC backend instead of RustCrypto RSA; Agent Factory GitHub authentication remains token-based and Sessions preserves its OIDC RS256 verification tests.
